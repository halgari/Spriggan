...
public class CFile
{
    public enum AbstractMethod : int
    {
        AbstractReader,
        AbstractWriter,
        ConcreteReader,
    ...
    public static List<(Type, AbstractMethod)> AbstractTypes = new();
    ...
    public CFile(GameRelease game)
    {
        ...
        _writerEmitters = new()
        {
            ...
            (typeof(IFormLink<>), IFormLinkWriter),
    ...
    private void MajorRecordInternalReader(Type arg1, string arg2, Context arg3)
    {
    ...
    private void Array2dReader(Type type, string getter, Context ctx)
    {
        ...
        var fn = GetItem();
        SB.AppendLine($"{TypeToCS(tinner)} {fn}(ref Utf8JsonReader reader)");
        using (SB.CurlyBrace())
        {
            var itm = GetItem();
        ...
        using (SB.CurlyBrace())
        {
            ...
            using (SB.CurlyBrace())
            {
                ...
                using (SB.CurlyBrace())
                {
                    ...
                    {
                        SB.AppendLine(
                            $"{maleName} = new FormLinkNullable<{itypeCS}>(SerializerExtensions.ReadFormKeyValue(ref reader, options));");
                        SB.AppendLine("break;");
                    }
                    ...
                    {
                        SB.AppendLine(
                            $"{femaleName} =  new FormLinkNullable<{itypeCS}>(SerializerExtensions.ReadFormKeyValue(ref reader, options));");
                        SB.AppendLine("break;");
                    }
                }
            }
    ...
    {
        var allTypes = info.Assembly.GetTypes().Where(t => t.InheritsFrom(info) && !t.IsAbstract && t.IsPublic)
            .ToArray();
    ...
    {
        var allTypes = type.Assembly.GetTypes().Where(t => t.InheritsFrom(type) && !t.IsAbstract && t.IsPublic)
            .ToArray();
    ...
    private void EnumReader(Type type, string getter, Context ctx)
    {
        if (type.CustomAttributes.Any(a => a.AttributeType == typeof(FlagsAttribute)))
            SB.AppendLine(
                $"{getter} = SerializerExtensions.ReadFlags<{CleanName(type.FullName)}>(ref reader, options);");
        else
            SB.AppendLine(
                $"{getter} = SerializerExtensions.ReadEnum<{CleanName(type.FullName)}>(ref reader, options);");
    }
    ...
    public void EmitAbstractClassWriter(Type tp)
    {
        var allTypes = Inheritors(tp);
        ...
        using (SB.CurlyBrace())
        {
            ...
            using (SB.CurlyBrace())
            {
                ...
                using (SB.CurlyBrace())
                {
                    ...
                    {
                        SB.AppendLine($"var {itm} = SerializerExtensions.ReadTag(ref reader, $\"$type\", options);");
                        SB.AppendLine($"switch({itm})");
                    }
    ...
    public void EmitConcreteClassReader(Type type)
    {
        ...
        using (SB.CurlyBrace())
        {
            ...
            {
                SB.AppendLine(
                    $"public static {TypeToCS(type)} ReadInner(ref Utf8JsonReader reader, FormKey formKey, JsonSerializerOptions options)");
            }
            else
            {
                SB.AppendLine(
